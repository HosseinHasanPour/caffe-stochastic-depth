...
template <typename Dtype>
void NesterovSolver<Dtype>::ComputeUpdateValue_StochDep(int param_id, Dtype rate) {
  ...
  switch (Caffe::mode()) {
    case Caffe::CPU: {
      ...
      caffe_cpu_axpby(learnable_params[param_id]->count(), Dtype(1) + momentum,
    ...
    case Caffe::GPU: {
#ifndef CPU_ONLY
      nesterov_update_gpu(learnable_params[param_id]->count(),
